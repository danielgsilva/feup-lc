...
struct packet pp;
MouseState mouse_state = {150, 150, false}; // {x, y, mouse_mv}
...
void(mouse_ih)(void) {
  ...
  if (kbc_read_out_buf(&mouse_packet_byte, 1) != 0) {
    //printf("error mouse_ih(): kbc_read_out_buf()\n");
    return;
  }
  // bit 3 of byte 1 of the packet must be 1 (Synchronization Issues)
...
int(mouse_data_reporting)(bool enable) {
  ...
  while (attempts) {
    ...
    if (util_sys_inb(KBC_OUT_BUF, &ack_byte) != 0)
      return 1;
